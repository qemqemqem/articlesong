...
// Load prompts using importScripts (Service Worker compatible)
...
// Request tracking
const activeRequests = new Map(); // id -> { request, abortController }
...
async function loadAPIKeys() {
  const result = await chrome.storage.sync.get(['anthropic_api_key', 'suno_api_key']);
...
async function saveRequests() {
  await chrome.storage.local.set({ requests: allRequests });
  broadcastUpdate();
}
...
function broadcastUpdate() {
  chrome.runtime.sendMessage({ action: 'requestsUpdated' }).catch(() => {
...
async function waitForSunoLyrics(taskId, abortController) {
  ...
  // Service Worker compatible: use async while loop instead of setInterval
    ...
    try {
      ...
      if (statusData.status === 'SUCCESS' && statusData.response && statusData.response.data) {
        ...
        if (!lyricsVariations || lyricsVariations.length === 0) {
          throw new Error('SunoAPI returned SUCCESS but no lyrics data');
        }
        ...
        return selectedLyrics.text;
      } else if (statusData.status === 'FAILED') {
        Logger.error('SunoAPI lyrics generation failed', { statusData });
        throw new Error('SunoAPI lyrics generation failed');
      }
  ...
  throw new Error('SunoAPI lyrics generation timeout (2 minutes)');
}
        ...
        if (!streamingUrlDelivered && song.audioUrl) {
          ...
          if (settings.showNotifications) {
            chrome.notifications.create({
              type: 'basic',
              iconUrl: chrome.runtime.getURL('icons/songify.png'),
...
async function generateSongFromArticle(request) {
  ...
  activeRequests.set(request.id, { request, abortController });
...
async function forwardAudioUrlToContentScript(tabId, audioUrl, requestId) {
  ...
  try {
    const isReady = await chrome.tabs.sendMessage(tabId, {action: "ping"}).catch(() => false);
...
function openSettingsWithError(errorType) {
  ...
  chrome.storage.local.set({ settingsError: errorType }).then(() => {
...
// Track if we've already checked for stuck requests this session
let stuckRequestsChecked = false;

// Helper to ensure state is loaded (Service Worker can restart anytime)
async function ensureStateLoaded() {
  // If state appears empty, reload from storage
  if (!SUNO_API_KEY) {
    await loadAPIKeys();
  }
  ...
  // Check for stuck requests ONLY ONCE per service worker lifecycle
  // Don't check on every getRequests call - only on first load after restart
  if (!stuckRequestsChecked) {
    stuckRequestsChecked = true;
    
    // Check for requests that were stuck mid-generation when service worker restarted
    // These async operations were lost when the worker was terminated
    ...
    if (stuckRequests.length > 0) {
      Logger.warning(`Found ${stuckRequests.length} stuck request(s) from service worker restart`, {
        requests: stuckRequests.map(r => ({ id: r.id, status: r.status, title: r.articleTitle }))
      });
      ...
      await saveRequests();
      Logger.info('Stuck requests marked as failed - user can retry');
    }
  }
}
    ...
    (async () => {
      try {
        ...
        if (message.useSelection && message.selectedText) {
          ...
          const content = await chrome.tabs.sendMessage(message.tabId, {action: "getText"});
          if (!content) {
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  try {
    const content = await chrome.tabs.sendMessage(tab.id, {action: "getText"});
