...
function saveOptions(e) {
  ...
  const syncPromise = chrome.storage.sync.set({
    anthropic_api_key: anthropicKey,
    suno_api_key: sunoKey
  });
  
  // Save settings to local storage
  const localPromise = chrome.storage.local.get('settings').then((result) => {
...
function restoreOptions() {
  ...
  const syncPromise = chrome.storage.sync.get(['anthropic_api_key', 'suno_api_key']).then((result) => {
  ...
  const localPromise = chrome.storage.local.get(['settings', 'settingsError']).then((result) => {
    const settings = result.settings || {};
    ...
    if (result.settingsError) {
      ...
      chrome.storage.local.remove('settingsError');
    }
  });
...
function openStyleEditor() {
  chrome.tabs.create({ url: chrome.runtime.getURL('styles.html') });
}
